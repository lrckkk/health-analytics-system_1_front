<template>
  <div class="main-container">
    ...
    <div class="right-column1">
      ...
      <div v-if="growthStore.historicalData['3']?.length > 0">
        <p>平均增长率:
          <span v-if="growthStore.getAverageGrowthRate('3') !== 0">
        ...
        <p v-if="growthStore.getEstimatedNextYearValue('3') !== null">
          估算 2021 年人均费用: {{ growthStore.getEstimatedNextYearValue('3')?.toFixed(2) }}
        </p>
        <p v-else>
          无法估算下一年的值 (数据不足)
        </p>
...
import ScatterChart from "@/components/ScatterChart.vue"; // 请确保路径正确，可能需要调整
// --- Pinia Stores ---
...
const resultByIdDisplay = computed(() => {
  const data = mapDataStore.totalCostData; // 获取人口数据
  console.log(data)
  // 关键校验：确保数据是数组且不为空
...
watch(
    ...
    async (newRegionId) => {
      ...
      try {
        // 直接调用 store 的 action，它会处理缓存逻辑
        const data = await regionStore.fetchcostavgIfNeeded(newRegionId);
        medicalcostData.value = data;
        // **关键修改：将从 fetchcostavgIfNeeded 获取的数据存储到 growthStore 的 '3' 号键下**
        growthStore.setHistoricalData('3', data);
      } catch (error) {
        console.error("在组件中处理数据获取失败:", error);
        medicalcostData.value = [];
        growthStore.setHistoricalData('3', []); // 错误时清空 '3' 键的数据
      } finally {
