  ...
  <div class="dashboard-container">
    ...
    <div class="dashboard-header">
      <div class="header-content">
        ...
        <h1 class="dashboard-title">{{regionStore.selectedRegion}} - 医疗费用分析</h1>
    ...
    <div class="chart-grid">
      <!-- 人均医疗费用折线图 -->
      <div class="chart-card line-chart" v-if="medicalcostData.length > 0">
        <simpleline
            ref="avgChart"
            :chart-data="medicalcostData"
            :title="`人均医疗费用 - ${regionStore.getDisplayRegion}`"
            height="400px"
            width="100%"
            x-field="year"
            y-field="count"
      ...
      <!-- 医疗费用年度分布散点图 -->
      <div class="chart-card scatter-chart" v-if="costData.length > 0">
        <scatter-chart
            ref="scatterChart"
            :chart-data="costData"
            :title="`医疗费用年度分布 - ${regionStore.getDisplayRegion}`"
            height="400px"
            width="100%"
            color="#00B4D8"
        />
      ...
      <!-- 费用组成饼图 -->
      <div class="chart-card pie-chart" v-if="costDistributionData.length > 0">
        <pie-chart
            ref="pieChart"
            width="100%"
            :chart-data="costDistributionData"
            :title="`费用组成 - ${regionStore.getDisplayRegion}`"
            name-field="category"
            value-field="value"
            height="450px"
            rose-type="radius"
...
const costData = ref([]);
const costDistributionData = ref([
  { category: '门诊费用', value: 35 },
  { category: '住院费用', value: 45 },
  { category: '药品费用', value: 15 },
  { category: '检查费用', value: 5 },
]);
...
const stats = computed(() => [
  ...
  {
    label: '平均增长率',
    value: growthStore.getAverageGrowthRate('3') !== 0
        ? `${(growthStore.getAverageGrowthRate('3') * 100).toFixed(2)}%`
        : '数据不足或增长率为零'
  },
  {
    label: '下一年估算值',
    value: growthStore.getEstimatedNextYearValue('3') !== null
        ? growthStore.getEstimatedNextYearValue('3')?.toFixed(2)
        : '无法估算(数据不足)'
  }
]);
...
const loadCharts = async () => {
  try {
    ...
    const [avgData, trendData] = await Promise.all([
      regionStore.fetchcostavgIfNeeded(regionStore.getRegionId),
      regionStore.fetchCostDataIfNeeded(regionStore.getRegionId)
    ]);

    medicalcostData.value = avgData;
    costData.value = trendData;
    growthStore.setHistoricalData('3', avgData);
    ...
    if (scatterChart.value) scatterChart.value.initChart();
